...
TEST_F(LibWaypointFollowerTestSuite, calcRelativeCoordinate)
{
  ...
  for (const auto& el : dataset)
  {
    ...
    const CoordinateResult ret =
        (x < 0.0) ? CoordinateResult::Negative : (x > 0.0) ? CoordinateResult::Positive : CoordinateResult::Equal;
    ASSERT_EQ(ret, el.second.second) << "Failure in " << el.first << ", it must be "
                                     << static_cast<int>(el.second.second) << ".";
  }
}
...
TEST_F(LibWaypointFollowerTestSuite, inDrivingDirection)
{
  ...
  for (const auto& el : dataset)
  {
    ...
    ASSERT_EQ(ret, el.second.second) << "Failure in " + el.first << ", it must be " << el.second.second << ".";
  }
}
